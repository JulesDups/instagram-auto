...
Prompt à coller dans un **trigger Claude Code** (cron) qui génère un carousel Instagram par run et le persiste dans la base Neon Postgres de l'app via HTTP.

Prérequis :

1. **`INTAKE_SECRET`** copié depuis `.env.local` — à stocker en variable d'env du trigger (jamais en dur dans le prompt).
2. **URL publique du déploiement** : `https://instagram-auto.vercel.app` (ou l'alias de production actuel).
3. **L'agent dispose de l'outil Bash** avec `curl` — aucun MCP GitHub n'est nécessaire, l'app est désormais DB-backed.
...
## Tester le prompt à la main avant de planifier

Avant de programmer le run quotidien :

1. Exporte temporairement `INTAKE_SECRET` et `PUBLIC_BASE_URL` dans ton shell.
2. Lance le prompt dans Claude Code en mode one-shot avec `claude -p "<prompt>"` ou via un trigger manuel.
3. Vérifie que :
   - `GET /api/next-source` renvoie bien un JSON avec un `kind`
   - Le draft généré valide le `DraftSchema` (pas de 400 au POST)
...
Si tout passe, planifie le trigger à `0 8 * * *` (8h00 chaque jour, fuseau Europe/Paris).

---

## Cadence et garde-fous

- 4–5 runs par semaine. Planifier du lundi au vendredi est suffisant pour atteindre la cible.
